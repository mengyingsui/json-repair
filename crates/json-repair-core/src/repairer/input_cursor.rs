...
mod tests {
    ...
    fn char_at_in_bounds() {
        let c = InputCursor::new("hello");
        assert_eq!(c.char_at(0), 'h');
        assert_eq!(c.char_at(4), 'o');
    }

    #[test]
    fn char_at_out_of_bounds_returns_nul() {
        let c = InputCursor::new("hi");
        assert_eq!(c.char_at(2), '\0');
        assert_eq!(c.char_at(100), '\0');
    }
    ...
    fn char_at_multibyte_utf8() {
        ...
        assert_eq!(c.char_at(0), '\u{00A0}');
        assert_eq!(c.char_at(2), '\u{1F600}');
        assert_eq!(c.char_at(6), '!');
    }
