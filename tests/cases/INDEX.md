...
| `edge_cases.jsonl`               | 67    | Empty objects/arrays, bare strings/numbers/booleans, multi-byte Unicode (Chinese), special symbols, Windows backslash paths, backslash-at-EOF, surrogate escapes, `//` comments + truncated closing brackets + duplicate keys, leading-zero numbers, multi-E/invalid-`\u` fuzzer regressions.     |
