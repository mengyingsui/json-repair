...
All `.jsonl` files under `tests/cases/` are auto-discovered by the Rust
integration test `crates/json-repair-core/tests/jsonl_cases.rs::test_all_jsonl_cases`
(lines with `"expected"`) — each line's `repair_json()` output is validated
against the expected value. Lines use the format: `{"input": "...", "expected": ...}`.
...
**Lines without `"expected"`**: still tested by `jsonl_cases.rs` — the test
calls `repair_json()` and verifies the output is **valid JSON** (parseable by
`serde_json`), but does **not** assert a specific output value. This is used
for fuzzer regression inputs with non-deterministic output and for large
real-world inputs where writing an exact `expected` value would be impractical.
Additionally, `broken_patterns.jsonl` is tested by its own dedicated test
(`broken_patterns.rs`) for validity + idempotence, and `bench_data.jsonl` is
benchmark-only (no test assertions at all).
...
| `edge_cases.jsonl`               | 67    | Empty objects/arrays, bare strings/numbers/booleans, multi-byte Unicode (Chinese), special symbols, Windows backslash paths, backslash-at-EOF, surrogate escapes, `//` comments + truncated closing brackets + duplicate keys, leading-zero numbers, multi-E/invalid-`\u` fuzzer regressions, number colon fallback. |
...
| `unescaped_quotes.jsonl`         | 12    | Unescaped " inside strings — parser deduces delimiter vs content. Handles `"Stop Doing"`, `"Pomodoro Timer"`, `"Done"` in long text values; also `"The Climb"` followed by `,` and `"buddy movie"` in prose. Plus embedded quotes before `]`/`}` with bracket-stack + string-aware balance scan. Case 12: embedded quote preceded by `{` inside a value string (backward-scan guard). |
| `unquoted_keys.jsonl`            | 4     | {key: "value"} style unquoted object keys. Multiline key with newline before `:`. |
| `unquoted_values.jsonl`          | 10    | Unquoted string values like {"name": John} — also multi-word values with spaces, values containing escaped quotes; multiline unquoted value with newline; URL with colon preserved.                                                                                                                                                                               |
