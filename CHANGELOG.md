...
- `repair_json()` now makes **6 input passes** (trim, serde_json fast-path,
  preprocess_json, normalize_preamble, is_implicit_object_sequence, main loop)
  and **0 output passes** (bracket_depth check is O(1)).

### Python
- `json-repair` bumped to **v0.4.1**.

### Rust
- Workspace `json-repair-core` bumped to **v0.2.1**.

## v0.4.0 🔒 (2026-07-12)

### BREAKING
- **Removed `fix_colon_in_key` and `fix_mixed_quotes`** from
  `json_repair_core`'s public API.  The fused `preprocess_json` now handles
  both transforms internally.  Call `repair_json` instead.
...
- **Single-pass preprocessor fusion** (`preprocess.rs`) — `preprocess_json`
  ...
  bytes. The internal helpers `needs_colon_fix`/`fix_colon_in_key`/
  `fix_mixed_quotes` are replaced by `try_mixed_quote_boundary`/
...
- **`tests/preprocess.rs` deleted** — standalone preprocessor tests removed
  (functions are private; coverage via JSONL test cases).
- **`tests/README.md`** — updated to reflect preprocess test deletion.
- **`is_comment_start`** (`comment.rs`) — doc fix: `recognised` → `recognized`.
- Rust crate `json-repair-core` bumped to **v0.2.0**.
- Workspace `Cargo.toml` bumped to **v0.2.0**.
...
- **`test_comma_separated_objects`** (`scenario.rs`) — comma-separated
  `{…},{…}` is now correctly asserted as an array (20 elements),
  was incorrectly checking `result.is_object()`.
...
- **`is_output_balanced` validation** (`repairer.rs`) — called at end of `repair()`
  in all build profiles; `repair_json` returns `Err` on imbalance (API guarantee).
...
### Python
- **`[tool.uv] package = false`** (`pyproject.toml`) — prevents `uv sync` from
  creating a competing editable install; `maturin develop` is now the sole install
  mechanism (matches pydantic-core pattern).
- **Dev workflow updated** — `uv run maturin develop --uv` replaces
  `uv run maturin develop --release -m crates/json-repair-python/Cargo.toml`;
  stale `.pyd` manual copy no longer needed.  `AGENTS.md` and `README.md` updated.
- **Pre-commit pytest hook** (`.pre-commit-config.yaml`) — runs
  `uv run pytest tests/python/ --no-header -q`.
...
- **JSONL auto-discovery refactored** (`helpers.rs`) — `broken_patterns.jsonl`,
  ...
  each has dedicated Rust tests.
- **Overlapping coverage eliminated** — coverable static inputs migrated from
  `edge_cases.rs` to JSONL; duplicate JSONL rows removed.
- **`tests/README.md` updated** — corrected `jsonl_cases.rs` description; added
  `proptests.rs` entry.
- **`tests/cases/INDEX.md` updated** — documented that lines without `expected`
  still require valid-JSON output (no specific value assertion).
